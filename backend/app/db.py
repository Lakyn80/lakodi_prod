...
def _ensure_invoice_search_norm_columns():
    """Add Phase 1 hybrid search normalized columns and indexes (SQLite)."""

    if not DATABASE_URL.startswith("sqlite"):
        return
    with engine.begin() as conn:
        invoices_exists = conn.execute(
            text("SELECT name FROM sqlite_master WHERE type='table' AND name='invoices'")
        ).fetchone()
        if invoices_exists:
            invoice_cols = {
            ...
            invoice_add_map = {
                "customer_name_search_norm": (
                    "ALTER TABLE invoices ADD COLUMN customer_name_search_norm VARCHAR(256)"
                ),
                "invoice_number_norm": (
                    "ALTER TABLE invoices ADD COLUMN invoice_number_norm VARCHAR(64)"
                ),
                "variable_symbol_norm": (
                    "ALTER TABLE invoices ADD COLUMN variable_symbol_norm VARCHAR(9)"
                ),
                "customer_ico_norm": (
                    "ALTER TABLE invoices ADD COLUMN customer_ico_norm VARCHAR(32)"
                ),
                "customer_dic_norm": (
                    "ALTER TABLE invoices ADD COLUMN customer_dic_norm VARCHAR(32)"
                ),
            }
            for col, sql in invoice_add_map.items():
                if col not in invoice_cols:
                    conn.execute(text(sql))
            conn.execute(
                text(
                    "CREATE INDEX IF NOT EXISTS ix_invoices_customer_name_search_norm "
                    "ON invoices (customer_name_search_norm)"
                )
            )
            conn.execute(
                text(
                    "CREATE INDEX IF NOT EXISTS ix_invoices_invoice_number_norm "
                    "ON invoices (invoice_number_norm)"
                )
            )
            conn.execute(
                text(
                    "CREATE INDEX IF NOT EXISTS ix_invoices_variable_symbol_norm "
                    "ON invoices (variable_symbol_norm)"
                )
            )
            conn.execute(
                text(
                    "CREATE INDEX IF NOT EXISTS ix_invoices_customer_ico_norm "
                    "ON invoices (customer_ico_norm)"
                )
            )
            conn.execute(
                text(
                    "CREATE INDEX IF NOT EXISTS ix_invoices_customer_dic_norm "
                    "ON invoices (customer_dic_norm)"
                )
            )

        subjects_exists = conn.execute(
            text(
                "SELECT name FROM sqlite_master WHERE type='table' AND name='invoice_subjects'"
            )
        ).fetchone()
        if subjects_exists:
            subject_cols = {
            ...
            subject_add_map = {
                "name_search_norm": (
                    "ALTER TABLE invoice_subjects ADD COLUMN name_search_norm VARCHAR(256)"
                ),
                "ico_norm": "ALTER TABLE invoice_subjects ADD COLUMN ico_norm VARCHAR(32)",
                "dic_norm": "ALTER TABLE invoice_subjects ADD COLUMN dic_norm VARCHAR(32)",
                "email_norm": (
                    "ALTER TABLE invoice_subjects ADD COLUMN email_norm VARCHAR(256)"
                ),
            }
            for col, sql in subject_add_map.items():
                if col not in subject_cols:
                    conn.execute(text(sql))
            conn.execute(
                text(
                    "CREATE INDEX IF NOT EXISTS ix_invoice_subjects_name_search_norm "
                    "ON invoice_subjects (name_search_norm)"
                )
            )
            conn.execute(
                text(
                    "CREATE INDEX IF NOT EXISTS ix_invoice_subjects_ico_norm "
                    "ON invoice_subjects (ico_norm)"
                )
            )
            conn.execute(
                text(
                    "CREATE INDEX IF NOT EXISTS ix_invoice_subjects_dic_norm "
                    "ON invoice_subjects (dic_norm)"
                )
            )
            conn.execute(
                text(
                    "CREATE INDEX IF NOT EXISTS ix_invoice_subjects_email_norm "
                    "ON invoice_subjects (email_norm)"
                )
            )


def backfill_invoice_search_norms(*, batch_size: int = 200) -> dict[str, int]:
    """Idempotent backfill of normalized search columns for subjects and invoices.

    Only updates rows where a computed norm differs from the stored value (including NULL).
    Uses short transactions per batch for SQLite safety.
    """
    ...
    while True:
        ...
        try:
            subjects = (
                db.query(InvoiceSubject)
                .filter(InvoiceSubject.id > last_subject_id)
                .order_by(InvoiceSubject.id.asc())
                .limit(batch_size)
                .all()
            )
            if not subjects:
            ...
            for subject in subjects:
                before = (
                    subject.name_search_norm,
                    subject.ico_norm,
                    subject.dic_norm,
                    subject.email_norm,
                )
                apply_subject_search_norms(subject)
                after = (
                    subject.name_search_norm,
                    subject.ico_norm,
                    subject.dic_norm,
    ...
    while True:
        ...
        try:
            invoices = (
                db.query(Invoice)
                .filter(Invoice.id > last_invoice_id)
            ...
            for invoice in invoices:
                before = (
                    invoice.customer_name_search_norm,
                    invoice.invoice_number_norm,
                    invoice.variable_symbol_norm,
                    invoice.customer_ico_norm,
                    invoice.customer_dic_norm,
                )
                apply_invoice_search_norms(invoice)
                after = (
                    invoice.customer_name_search_norm,
                    invoice.invoice_number_norm,
                    invoice.variable_symbol_norm,
    ...
    return {"subjects_updated": subjects_updated, "invoices_updated": invoices_updated}
